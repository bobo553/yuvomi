...
**Events moved before that (#1307):** they show up in the backfill's confirmation above, one by one. `GET` returns them as `moved`: events whose only assignee is still the untouched default assignee (same rule as the move, `UNTOUCHED_DEFAULT_ASSIGNMENT`, plus `user_modified = 0`) of another calendar of the same provider - for CalDAV the same account via `caldav_calendar_selection` - sitting in a calendar with a different default assignee; each entry carries title, start, calendar name and the two persons. Only events the requesting admin may see under the calendar visibility rule (`visibilityWhere`, #474, no admin bypass) are listed, counted or changed; the #1154 `count`/`token` only count and name no event and stay unfiltered. The list comes in pages of at most 5000 entries (`movedCandidatesLimit`), oldest start first: `moved_total` counts all, `moved_offset` those before the page, and `moved_next` is a cursor (`<id>:<start_datetime>` of the last entry) that `GET ?moved_after=` takes for the next page, so a page left entirely unticked does not hide the rest. The dialog confirms per page and offers "Show next" to move on without applying. `POST` accepts at most as many `moves` as a page lists, so the default confirmation always fits, and a longer list answers 400 naming the limit; each move is checked by a point query against the same rule and visibility, without loading the list. The data cannot tell such an event from one that never moved but sits in a calendar whose default assignee was changed later while the previous person is another calendar's default: `event_assignments` has no origin or timestamp, `external_calendars` keeps no history, CalDAV and iCloud overwrite `external_object_url` on every inbound, and Google keeps the event id across a move. So nothing is changed globally: the admin unticks what should stay, and `POST` takes the picked entries as `moves: [{ event_id, from_user_id, to_user_id }]`. An entry that no longer matches exactly that way answers 409 with the first page; without `moves` no existing assignment changes. The picked events go through the same write as the move (`setEventAssignments()`, past inherited reminders dismissed) and are re-checked when written; a series master takes its linked occurrences along.
...
| receipt_document_id | INTEGER | FK → Family Documents (SET NULL on delete), nullable. Every serialized session carries `has_receipt` (`null` without read access to the Documents module - member right or token scope - so the visit does not say that it has one, #1358); the id (and in the visit list and report the name, `receipt_document_name`) go only to callers who may read that document - Documents access, token scope, document visibility (#1358) - otherwise `null`. On update, a caller who cannot see the stored receipt keeps it (`null` or a missing field change nothing, any id - the stored one included - is the same 403); a new link needs Documents access |
...
**Reversal (#1309).** A recorded payment is reversed, never deleted: `POST /api/v1/split-expenses/groups/:id/settlements/:settlementId/reverse` books the exact negative of every `settlement` ledger row as `settlement_reversal` (same `source_id`), in one transaction, so balances return to where they were before the payment. The settlement row, its settlement entries and `proof_document_id` stay untouched, and so does `status` - its second value means deleted, and a reversal is not a deletion. Whether and when a payment was reversed is read from those ledger rows (`created_at`, `created_by`), not from a column of its own: balances are computed from the ledger alone, and a second copy of the state could say "reversed" while the balance counted the payment again. Allowed for group owners/admins and for whoever recorded the payment - the same rule as editing or deleting an expense (`mayChangeGroupRecord`) - and, like every write under `/split-expenses`, only with `budget: write`. A second reversal answers 409; a reversal has no id of its own, so it cannot itself be reversed. Correcting a payment means reversing it and recording it again. The activity feed logs `payment_reversed`, and each `payment_registered` entry carries a `settlement` object (payer, payee, amount, `reversed_at`, `can_reverse` for the caller).
...
**Restored bookings (#1382).** Migration v226 writes one `ledger_restored` entry (`entity_type` 'expense', `actor_id` NULL, shown as "System") per active expense whose lost ledger rows it rebuilt. Its metadata is `{title, amount_minor, currency}` with the booked (converted) amount, because frozen migration SQL cannot apply per-currency minor units; the activity endpoint adds the decimal `amount` (ISO 4217), and the feed shows title and amount.

**Paging (#1309).** `GET /api/v1/split-expenses/groups/:id/activity` returns newest first, ordered by `created_at DESC, id ASC`. The `id` is part of the order because `created_at` has one-second resolution and several entries share a second; `id ASC` within a second is what the index `idx_expense_activity_group_created` already produced before, now written out so existing callers keep the same list. Every entry is reachable through a cursor: `pagination.next_cursor` (`{ before_at, before_id }`, null when `has_more` is false) names the last entry of the page, and passing both values returns the entries after it. Entries added meanwhile land at the top and shift nothing. `limit` defaults to 30 and is capped at 100. Without a cursor the endpoint behaves as before (`limit`, `offset`); a cursor together with a non-zero `offset`, or only one half of the cursor, answers 400. `has_more` is exact (one row more than requested is read). The feed in the UI loads 12 entries and appends 12 more per "Load more"; the button is reading, not an action, so it appears at every access level and in the archive. A refresh within the same group - after a reversal, a new expense - keeps the loaded depth by paging until the previously last entry is loaded again; switching groups starts at the first page, and a page that arrives after the switch is discarded. Entry dates are the household day of `created_at` (`zonedDateKey`), not its UTC date.
...
(docs/DECISIONS.md #8): Yuvomi stores the numbers a household states about its own meal and sums
them, and never turns an ingredient into nutrients. None of the columns is called `calories`,
because `health_activities.calories` is energy burnt.

`health_nutrition_targets` - one row per person (`user_id` primary key, cascading on user
deletion), the eight nutrients as nullable non-negative REAL columns, timestamps. The row is sparse:
a target with all eight values `NULL` is deleted instead of stored, so "no row" is the only way to
say "no target", and `0` stays an explicit target of zero.

`health_nutrition_entries` - the log:
...
| note | TEXT | Nullable |
| visibility | TEXT | `private` (default) or `all` - the canonical set of docs/DECISIONS.md #5, not the `private`/`family` pair of the older Health tables |
| created_by | INTEGER | Required; user deletion cascades |
| created_at, updated_at | TEXT | UTC audit timestamps |

The owner and a granted caregiver read every entry, everybody else only entries opened with `all`
(`careAwareClause()`, the one place that clause is built, #884). A target is not a row with a
visibility: only the owner and a caregiver who may record for them can read it.
...
- **Bulk archive (#1250):** `POST /api/v1/tasks/archive` (`{ ids }`, at most 500) files several tasks in one transaction under the rules of the single route: ids the caller cannot see drop out silently (not counted, since a count would confirm they exist), locked tasks the caller may not change - a subtask inherits its parent's lock - are left out and counted as `skipped`, and a task already archived keeps its first `archived_at`. Response: `{ data: { archived, skipped } }`. The client always sends the ids it shows, never "everything done", so a task completed elsewhere after the view was drawn is not filed unseen. Two entry points use it: the list's selection mode (previously one `PATCH /:id/archive` per task, which stopped at the first error and did not reload), and an archive button in the header of the Kanban "Done" column that, after a confirmation, files the done cards the column currently shows. More than 500 ids go in consecutive batches.
...
- **File attachments:** Events support a single file attachment (images, PDFs, Office documents, up to `MAX_UPLOAD_MB`, default 5 MB — the shared upload ceiling since v2.28.0, #806). Images are displayed inline in the event popup; other files show a download link. Drag-and-drop upload is supported in the event modal. New attachments create one `family_documents` object through the active document-storage backend and link it via `attachment_document_id`; no second binary copy is written to `attachment_data`. Existing legacy Base64 attachments remain readable. Unchanged attachments are not re-uploaded, and removing an attachment only unlinks it from the event. **The attachment follows the Documents right (#1358):** it is served only to a caller who may read the Documents module (member right, for API tokens `documents:read`) and see the document itself; everyone else gets the event without it - `attachment_document_id`, the preview and download URLs, `attachment_name`, `attachment_mime` and `attachment_size` are `null` on every calendar read path and the dashboard, and the event popup shows no attachment. **Uploading is a transfer into the Documents module** (DECISIONS.md entry 10): a new attachment needs write access to documents (member right and token scope), asked before the event lookup, and the event dialog shows the upload area only then (with read access a visible attachment keeps its preview and remove button, with none the field is gone). Replacing or removing an attachment needs read access and sight of the stored document; otherwise the same 403 and the attachment stays. A caller with read access who cannot see the stored document gets `attachment_locked: true` (no id, name or type) and both the event popup and the dialog show "Attachment present (private)" without name, preview, upload or remove, the same text housekeeping shows for such a receipt. Saving an event carries its visibility and assignees onto the document only as far as the caller may: with documents write access, sight of the document and the right to manage it (its creator, i.e. the event creator, or an admin, `canManageDocument()`) fully, otherwise only narrowing (`applyDocumentAccess()` in `server/services/document-access.js`). A split or detach copies the attachment only for such a caller; otherwise the successor or detached event has none and the original stays on the original series. The default-assignee sync of connected calendars (moving an event between two mapped calendars, and the admin backfill) never changes document rights, with one exception: when the event is visible to its assignees and the document is already shared with selected members, the new assignee is added to those shares. Otherwise the owner's settings stay exactly as they are - nothing becomes visible to the whole family, nothing private is opened, nothing is narrowed and no share is removed. A copy made on split or detach keeps its source's visibility and shares and belongs to the source's owner.
...
- **CardDAV multi-account sync:** connect multiple CardDAV servers (Nextcloud, iCloud, Radicale, Baikal); per-addressbook enable/disable via checkboxes, plus "enable all / disable all" once an account has more than one addressbook; read-only inbound sync, automatic on the `SYNC_INTERVAL_MINUTES` schedule plus a manual trigger. Accounts are **editable** (`PUT /api/v1/contacts/cardav/accounts/:id`) — a rotated password no longer means deleting and re-adding the account and losing the addressbook selection; leaving the password field empty keeps the stored one as long as the server (scheme, host, port) and the username stay the same - a different server or username without a new password is refused with `400 password_required` and nothing is saved, so a stored password never travels to a server it was not entered for (v2.69.1); a different path on the same server needs no password. A URL+username that already belongs to another account is rejected with `409`. API routes under `/api/v1/contacts/cardav/*`: create/update/delete accounts, test connections, discover/refresh addressbooks, toggle addressbook selection, sync contacts. **All of them are admin-only (v2.69.1),** enforced by a router-wide `requireAdmin` like the CalDAV account routes; before that only the settings page was admin-only, while the server checked nothing beyond the `contacts` module permission. A token needs an admin subject and the `contacts` scope. Reading contacts (`/api/v1/contacts`) and the scheduled background sync are not affected. Error responses carry a stable `errorCode` (e.g. `account_duplicate`, `account_not_found`, `password_required`) that the client translates; the `error` string itself is an English developer note (v1.34.0)
...
- **Nutrition (#1326):** a daily target per person for up to eight nutrients, every one optional, and a log of entries with a title, a time, an optional meal type and the same eight numbers. The tab shows today's total against the target, per nutrient; a nutrient without a target reads "no target set" rather than "0 of 0". Nothing is computed from recipes or products - the figures are what the household enters (docs/DECISIONS.md #8). An opt-in dashboard tile (`nutrition`, hidden by default, lockable per member like cycle and fasting) shows the same total from the same calculation. The visibility default per person lives under the `nutrition` scope of the Health visibility defaults.
...
- **Forgot password (v0.71.51, hardened in #847):** A "Forgot password?" link opens `/forgot-password`. The link is only shown when the server can actually deliver a reset mail: the public `GET /api/v1/version` response carries a `password_reset_enabled` flag (true when SMTP is configured **and** `BASE_URL` is set) and the login page gates the link on it, so it is never a dead end. On the reset page, entering a username or email always returns a generic "if an account exists…" response (anti-enumeration), regardless of whether the identifier matched a user or whether SMTP is configured. The response goes out before the lookup: resolving the account and sending the mail run afterwards in the background, so the response time does not reveal whether an account exists either. Jobs for the same account run one after another, at most one running and one waiting (a further request while one waits is dropped, since the waiting job issues the newest token anyway), so when requests overlap, the mail sent last carries the link that is still valid; the recipient's mail servers can still reorder delivery. An email identifier is compared against `contacts.email` of contacts that point to an account, with the same normalisation as SSO linking (surrounding whitespace and capitalisation ignored); it resolves only when exactly one account holds the address - guests of shared expenses do not count towards that, but a guest alone is still found - so members sharing one address reset by username. When it does match and the user has a linked email (`contacts.email`), a reset link `${BASE_URL}/reset-password?token=…` is emailed; the token is single-use and expires after 1 hour. `/reset-password` reads the token from the query string and sets a new password (min. 8 characters); on success, the token is consumed and other sessions for that user are invalidated. Requires an admin-configured SMTP server (Settings → Administration → Email) and the `BASE_URL` env var — reset links are only sent when `BASE_URL` is set, since the request `Host` header is never trusted for this purpose (prevents reset-link poisoning). API: `POST /api/v1/auth/forgot-password`, `POST /api/v1/auth/reset-password` (both public, rate-limited).
